...
annotations = open('/content/drive/My Drive/huyen/darknet/data/wider_face_split/wider_face_val_bbx_gt.txt')
...
f2=open('./results/comp4_det_test_face.txt')
...
while i < len(lines_2):
	...
	filepath=path+'/'+name[1].replace('jpg','txt')
	print('filepath:',filepath)
	f3=open(filepath,'a')
	img=lines_2[i].strip()
	img=img.split('/')
	f3.write(img[1].replace('.jpg','\n'))
	f3.write(num_face)
	#name = lines_1[j]
	...
	for numface in range(int(num_face)):
		...
		f3.write(str(int(left_x)) + ' ')
		f3.write(str(int(top_y)) + ' ')
		f3.write(str(int(w)) + ' ')
		f3.write(str(int(h)) + ' ')
		f3.write(str(score) + '\n')
	i=i+int(num_face)+2
